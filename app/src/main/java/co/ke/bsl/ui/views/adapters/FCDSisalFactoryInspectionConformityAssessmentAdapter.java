...
public class FCDSisalFactoryInspectionConformityAssessmentAdapter extends AbstractFragmentStepAdapter {

    private static final String CURRENT_STEP_POSITION_KEY = "";

    public FCDSisalFactoryInspectionConformityAssessmentAdapter(FragmentManager fm, Context context) {
        super(fm, context);
    }
    ...
    public Step createStep(int position) {
        if (position == 0) {
            final FCDSisalFactoryInspectionConformityAssessmentStep1 step = new FCDSisalFactoryInspectionConformityAssessmentStep1();
            Bundle b = new Bundle();
            b.putInt(CURRENT_STEP_POSITION_KEY, position);
            step.setArguments(b);
            return step;
        } else if (position == 1) {
            final FCDSisalFactoryInspectionConformityAssessmentStep2 step = new FCDSisalFactoryInspectionConformityAssessmentStep2();
            Bundle b = new Bundle();
            b.putInt(CURRENT_STEP_POSITION_KEY, position);
            step.setArguments(b);
            return step;
        } else if (position == 2) {
            final FCDSisalFactoryInspectionConformityAssessmentStep3 step = new FCDSisalFactoryInspectionConformityAssessmentStep3();
            Bundle b = new Bundle();
            b.putInt(CURRENT_STEP_POSITION_KEY, position);
            step.setArguments(b);
            return step;
    ...
    public StepViewModel getViewModel(@IntRange(from = 0) int position) {
        ...
        if (position == 0) {
            return new StepViewModel.Builder(context)
                    .setTitle(R.string.sisal_factory_inspection)
