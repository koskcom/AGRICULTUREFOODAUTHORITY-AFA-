...
import co.ke.bsl.receivers.DataBroadcastReceiver;

public class DataSetupService extends Service {

    private static final String CLASS_NAME = "DataSetupService";
    ...
    private DataBroadcastReceiver mDataBroadcastReceiver = new DataBroadcastReceiver();

    final IntentFilter broadcastIntentFilter = new IntentFilter("android.media.action.DISPLAY_NOTIFICATION");

    public DataSetupService(Intent intent) {
        this.intent = intent;
    }
    ...
    public void onCreate() {
        ...
        this.registerReceiver(mDataBroadcastReceiver, broadcastIntentFilter);
        Notification notification = new Notification.Builder(getApplicationContext()).build();
        startForeground(1, notification);
        uri = getString(R.string.baseURI);
        handler = new Handler(getMainLooper());
        runnable = new Runnable() {
            @Override
            public void run() {

                System.out.println("Service running ----------------------------- with data " + data);

//                refreshCouponPeriodicWork(data);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(null != intent && null != intent.getStringExtra("UserDirectorate")) {
            data = intent.getStringExtra("UserDirectorate");
        }

        System.out.println("Service starting ----------------------------- with data " + data);
    ...
    public void onDestroy() {
        this.unregisterReceiver(mDataBroadcastReceiver);
        super.onDestroy();
    }

    public class SynchData extends AsyncTask<Void, Void, Void>{
    ...
    public void refreshCouponPeriodicWork(String directorate) {
        //define constraints
    ...
    public boolean isInternetAvailable() {
        try {
            ...
            System.out.println("Intetnet Connection");
            return !ipAddr.equals("");

        } catch (Exception e) {
            System.out.println("No Intetnet Connection");
            return false;
        }
    }
